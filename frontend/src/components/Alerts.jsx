...
export default function Alerts({ data, loading, onFilterChange }) {
  ...
  const activeWarning = data.active_warning;
    ...
    <div className="space-y-6" data-testid="alerts-page">
          ...
          alerts.map((alert, i) => {
            ...
            return (
              ...
              >
                ...
                <Badge
                  className={`text-[10px] font-semibold border flex-shrink-0 ${styles.badge}`}
                >
                  {alert.level}
                </Badge>
              </div>
      ...
      {activeWarning && (
        <div
          ...
          className={`fixed bottom-16 md:bottom-0 left-0 md:left-16 lg:left-64 right-0 z-30 px-4 py-3 flex items-center justify-between ${
            activeWarning.level === "ALERT"
              ? "bg-[#EF4444]"
              : "bg-[#F59E0B]"
          }`}
        >
