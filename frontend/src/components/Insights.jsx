...
function AdvisoryCard({ advisory }) {
  return (
    ...
    >
      ...
      <p className="text-sm leading-relaxed text-[#111827]">{advisory}</p>
...
function PrimaryDriverCard({ primaryDriver, contributions }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm font-medium text-[#111827] mb-4">{primaryDriver}</p>
          ...
          <div key={key} className="space-y-1">
            ...
            <div className="h-2 bg-[#E5E7EB] rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${value}%`,
                  backgroundColor: value === maxContribution ? "#1A7A3C" : "#1A7A3C80",
                }}
              />
            </div>
          </div>
      ...
      <ul className="space-y-3">
        {actions.map((action) => (
          <li key={action} className="flex items-start gap-3">
            <CheckCircle2 size={18} className="text-[#1A7A3C] flex-shrink-0 mt-0.5" />
...
export default function Insights({ data, loading }) {
  ...
  return (
    <div className="space-y-6" data-testid="insights-page">
      ...
      <PrimaryDriverCard primaryDriver={data.primary_driver} contributions={data.driver_contributions} />
      <CropProfileCard crop={data.crop} cropOptions={data.crop_options} safeRanges={data.safe_ranges} />
