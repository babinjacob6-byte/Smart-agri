...
function DesktopSidebar({ currentPath, navigate, dashboardData }) {
  return (
    <aside
...
function MobileTopBar({ dashboardData }) {
  return (
    <header
...
function MobileBottomNav({ currentPath, navigate }) {
  return (
    <nav
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-lg transition-all duration-150 ${
                isActive ? "text-[#1A7A3C]" : "text-[#9CA3AF]"
              }`}
            >
...
function SimulateAlertFAB({ isSimulated, onSimulate, onReset }) {
  return (
    <button
...
export default function Layout({ children, isSimulated, onSimulate, onReset, dashboardData }) {
  ...
  return (
    <div className="min-h-screen bg-white" data-testid="app-layout">
      <DesktopSidebar currentPath={currentPath} navigate={navigate} dashboardData={dashboardData} />
      <MobileTopBar dashboardData={dashboardData} />

      <main className="md:ml-16 lg:ml-64 pt-[60px] md:pt-0 pb-20 md:pb-6 min-h-screen" data-testid="main-content">
      ...
      <SimulateAlertFAB isSimulated={isSimulated} onSimulate={onSimulate} onReset={onReset} />
    </div>
  );
}
